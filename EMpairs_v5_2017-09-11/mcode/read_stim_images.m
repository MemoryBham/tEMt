...
for kt = 1:length(params.Imf)
    ...
    [params.theImage{kt},~,ach] = imread(theImageLocation); % loads the alpha level and the RGB values for each pixel
    sz =size((params.theImage{kt}));
    
    %% resize the images
%     % this doesn't work on the experiment computer atm (02.12.19) because of a problem
%     % with the .mex file of imresize. The workaround includes changing the
%     % end-coordinates for the images in calculate_coordinates:
%     % params.baseRect = [0 0 params.stimSizeX params.stimSizeY];
%     if ~isequal(sz(1:2), [params.stimSizeX, params.stimSizeY]) % check if the image has the same size as required
%         oldIma = params.theImage{kt};
%         newIma = imresize(oldIma, [params.stimSizeY, params.stimSizeX])
%     end    
...
if isfield(params,'stim_mat')
    x = {};
    for it = 1:size(params.stim_mat.seq,2)
        x{it} = params.Imf(params.stim_mat.seq(1,it)).name;
    end
    if length(unique(x)) ~= size(params.stim_mat.seq,2)
        error('array must not contain duplicate images');
    end
end
return